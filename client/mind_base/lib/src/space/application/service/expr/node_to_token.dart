...
class NodeToToken {
  ...
  /// [ctx] 需要传入 [SpaceModel.ctxQueryMetaSign]
  String convert(IExprNodeDto expr, Map<String, dynamic> ctx) {
    late String r;
    // print("debug convert#start#  expr: $expr");
    switch (expr.runtimeType) {
      case ExprValueNodeDto:
        r = convertValueNode(expr as ExprValueNodeDto, ctx);
        break;
      case ExprNodeDto:
        r = convertExprNode(expr as ExprNodeDto, ctx);
        break;
      default:
  ...
  String convertExprNode(ExprNodeDto expr, Map<String, dynamic> ctx) {
    late String r;
    final fnName = expr.fn;

    /// member表达式
    if (fnName == IFunc.memberFunc.name) {
      final mainAg = expr.ag[0]; // 调用方,可以是Expr或ExprValue
      final attrAg = expr.ag[1]; // 属性
      print("debug mainAg: $mainAg, attrAg: $attrAg");
      // 对member表达式进行特殊处理:
      r = convertMemberExprNode(mainAg, attrAg, ctx);
    } else {
  ...
  String convertMemberExprNode(
      IExprNodeDto mainAg, IExprNodeDto attrAg, Map<String, dynamic> ctx) {
    final main = convert(mainAg, ctx);
    // final mainMeta = mainAg.type.meta!;
    final mainMeta =
        (ctx[SpaceModel.ctxQueryMetaSign] as QueryMetaFn).call(mainAg.type!.r!);

    final propId = attrAg.ag[0]; // attr必定是 propId, 不过type可能是ref,或者是String
    final attrName = mainMeta.getPropBy(propId).name;
    return "$main.$attrName";
  }

  @protected
  String convertValueNode(
      ExprValueNodeDto exprValue, Map<String, dynamic> ctx) {
